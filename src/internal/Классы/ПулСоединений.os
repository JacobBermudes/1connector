...
Процедура ПриСозданииОбъекта()
	...
	ПулСоединений = Новый СинхронизированнаяКарта(Новый КартаСоответствие());
...
Функция Получить(ПараметрыСоединения) Экспорт
	...
	ЭлементПула = ПулСоединений.ПолучитьИлиУмолчание(ИдентификаторСоединения, Неопределено);

	Если ЭлементПула = Неопределено Тогда
	...
	Иначе
		Соединение = ЭлементПула.Соединение;
		ЭлементПула.Использовано = ТекущаяУниверсальнаяДата();
	КонецЕсли;
...
Процедура ЗакрытьСтарыеСоединения()
	...
	Для Каждого КлючЗначение Из ПулСоединений.КлючиИЗначения() Цикл
